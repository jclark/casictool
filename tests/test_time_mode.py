...
from casic import build_cfg_tmode, parse_cfg_tmode
...
class TestBuildCfgTmode:
    def test_auto_mode(self) -> None:
        ...
        mode, reserved = struct.unpack("<HH", payload[:4])
        assert mode == 0
        assert reserved == 0
    ...
    def test_survey_mode(self) -> None:
        ...
        assert len(payload) == 40
        mode, reserved, _, _, _, _, svin_dur, svin_var = struct.unpack("<HHdddfIf", payload)
        assert mode == 1
        assert reserved == 0
    ...
    def test_fixed_mode(self) -> None:
        ...
        assert len(payload) == 40
        mode, reserved, x, y, z, var, _, _ = struct.unpack("<HHdddfIf", payload)
        assert mode == 2
        assert reserved == 0
    ...
    def test_default_values(self) -> None:
        ...
        mode, reserved, _, _, _, pos_var, svin_dur, svin_var = struct.unpack("<HHdddfIf", payload)
        assert mode == 1
        assert reserved == 0
    ...
    def test_fixed_mode_negative_coords(self) -> None:
        """Mode 2 (fixed) correctly encodes negative ECEF coordinates."""
        ecef = (-1144698.0455, 6090335.4099, 1504171.3914)
        payload = build_cfg_tmode(
            mode=2,
            fixed_pos=ecef,
            fixed_pos_acc=10.0,
        ...
        assert mode == 2
        assert reserved == 0
        assert x == pytest.approx(-1144698.0455)
        assert y == pytest.approx(6090335.4099)
        assert z == pytest.approx(1504171.3914)
        assert var == pytest.approx(100.0)  # 10^2


class TestParseCfgTmode:
    def test_round_trip_fixed_mode(self) -> None:
        """Build and parse round-trip preserves values."""
        ecef = (-1144698.0455, 6090335.4099, 1504171.3914)
        payload = build_cfg_tmode(
            mode=2,
            fixed_pos=ecef,
        ...
        parsed = parse_cfg_tmode(payload)
        assert parsed.mode == 2
        assert parsed.fixed_pos_x == pytest.approx(-1144698.0455)
        assert parsed.fixed_pos_y == pytest.approx(6090335.4099)
        assert parsed.fixed_pos_z == pytest.approx(1504171.3914)
        assert parsed.fixed_pos_var == pytest.approx(100.0)

    def test_round_trip_survey_mode(self) -> None:
        """Build and parse round-trip for survey mode."""
        payload = build_cfg_tmode(
            mode=1,
            survey_min_dur=3600,
            survey_acc=25.0,
        ...
        assert parsed.svin_min_dur == 3600
        assert parsed.svin_var_limit == pytest.approx(625.0)  # 25^2

    def test_round_trip_auto_mode(self) -> None:
        """Build and parse round-trip for auto mode."""
        payload = build_cfg_tmode(mode=0)
        parsed = parse_cfg_tmode(payload)
